...
//--------------------------------------------------------------------------------
bool UdpManager::Setup(const UdpSettings &settings)
{
    if (!settings.bindPort && !settings.sendPort) {
        LOG(ERROR)
            << "passed settings object was not initialized with .receiveOn or .sendTo";
        return false;
    }
    ...
    if (settings.receiveBufferSize)
        SetReceiveBufferSize(settings.receiveBufferSize);
    if (settings.sendBufferSize)
...
//--------------------------------------------------------------------------------
int UdpManager::WaitReceive(time_t timeoutSeconds, time_t timeoutMicros)
{
    if (m_hSocket == INVALID_SOCKET)
        return SOCKET_ERROR;
...
//--------------------------------------------------------------------------------
int UdpManager::WaitSend(time_t timeoutSeconds, time_t timeoutMicros)
{
    if (m_hSocket == INVALID_SOCKET)
        return SOCKET_ERROR;
...
bool UdpManager::BindMcast(char *pMcast, unsigned short usPort)
{
    // bind to port
    if (!Bind(usPort)) {
        LOG(WARNING) << "BindMcast(): couldn't bind to port " << usPort;
        return false;
    }
    ...
    if (setsockopt(m_hSocket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char FAR *)&mreq, sizeof(mreq))
        == SOCKET_ERROR) {
        ofxNetworkCheckError();
        return false;
    }
...
bool UdpManager::Connect(const char *pHost, unsigned short usPort)
{
    ...
    // ofLogNotice("UdpManager") << "Connect(): connected to " << inet_addr( pHost );
...
bool UdpManager::ConnectMcast(char *pMcast, unsigned short usPort)
{
    ...
    if (!Bind(usPort)) {
#ifdef _DEBUG
        LOG(ERROR) << "ConnectMcast(): couldn't bind to " << usPort;
        ofxNetworkCheckError();
#endif
        return false;
    }
    ...
    if (!SetTTL(1)) {
#ifdef _DEBUG
        LOG(WARNING) << "ConnectMcast(): couldn't set TTL; continuing anyway";
        ofxNetworkCheckError();
#endif
    }
...
#ifdef _DEBUG
        LOG(ERROR) << " ConnectMcast(): couldn't connect to socket";
        ofxNetworkCheckError();
#endif
        return false;
    }
...
//	we can use MSG_PEEK, but we still need a large buffer (udp protocol max is 64kb even if max for
//this socket is less)
//	don't want a 64kb stack item here, so instead read how much can be read (note: not queue size,
//there may be more data-more packets)
#ifdef TARGET_WIN32
...
int UdpManager::Receive(char *pBuff, const int iSize)
{
    ...
    if (ret > 0) {
        // ofLogNotice("UdpManager") << "received from: " << inet_ntoa((in_addr)saClient.sin_addr);
        canGetRemoteAddress = true;
    }
    else {
        canGetRemoteAddress = false;

        //	if the network error is WOULDBLOCK, then return 0 instead of SOCKET_ERROR as it's not
        //really a problem, just no data.
...
void UdpManager::SetTimeoutSend(int timeoutInSeconds) { m_dwTimeoutSend = timeoutInSeconds; }

void UdpManager::SetTimeoutReceive(int timeoutInSeconds) { m_dwTimeoutReceive = timeoutInSeconds; }

int UdpManager::GetTimeoutSend() { return m_dwTimeoutSend; }

int UdpManager::GetTimeoutReceive() { return m_dwTimeoutReceive; }

//--------------------------------------------------------------------------------
bool UdpManager::GetRemoteAddr(std::string &address, int &port) const
{
    if (m_hSocket == INVALID_SOCKET)
        return (false);
    if (canGetRemoteAddress == false)
        return (false);
...
//--------------------------------------------------------------------------------
bool UdpManager::GetListenAddr(std::string &address, int &port) const
{
    if (m_hSocket == INVALID_SOCKET)
        return (false);
...
bool UdpManager::SetReceiveBufferSize(int sizeInByte)
{
    ...
    if (setsockopt(m_hSocket, SOL_SOCKET, SO_RCVBUF, (char *)&sizeInByte, sizeof(sizeInByte))
        == 0) {
        return true;
    }
    else {
...
bool UdpManager::SetSendBufferSize(int sizeInByte)
{
    ...
    if (setsockopt(m_hSocket, SOL_SOCKET, SO_SNDBUF, (char *)&sizeInByte, sizeof(sizeInByte))
        == 0) {
        return true;
    }
    else {
...
int UdpManager::GetTTL()
{
    ...
    if (getsockopt(m_hSocket, IPPROTO_IP, IP_MULTICAST_TTL, (char FAR *)&nTTL, &nSize)
        == SOCKET_ERROR) {
#ifdef _DEBUG
        LOG(ERROR) << "GetTTL(): getsockopt failed";
#endif
        ofxNetworkCheckError();
        return -1;
    }
...
bool UdpManager::SetTTL(int nTTL)
{
    ...
    if (setsockopt(m_hSocket, IPPROTO_IP, IP_MULTICAST_TTL, (char FAR *)&nTTL, sizeof(int))
        == SOCKET_ERROR) {
#ifdef _DEBUG
        LOG(ERROR) << "SetTTL(): setsockopt failed";
#endif
        ofxNetworkCheckError();
        return false;
    }
