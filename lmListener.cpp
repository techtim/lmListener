...
// #ifdef NDEBUG
//     /// setup log level, turn off verbose for release
//     el::Configurations conf.setToDefault();
//     el::Configuration confDisableVerbose(el::Level::Verbose, el::ConfigurationType::Enabled, "false");
//     std::string logerName = "default";
//     el::Loggers::reconfigureLogger(logerName, confDisableVerbose);
// #endif
        ...
        if ((received = frameInput.PeekReceive()) > 4) {
            ...
            if (isWS) {
                ...
                for (curChannel = 0; curChannel < chan_cntr; ++curChannel) {
                    ...
                    LOG(DEBUG) << "Send SPI chan: " << curChannel << " leds: " << ledsInChannel[curChannel];
